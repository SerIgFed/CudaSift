...
double ScaleDown(const SiftData &siftData, CudaImage &res, CudaImage &src, float variance)
{
...
double ScaleUp(const SiftData &siftData, CudaImage &res, CudaImage &src)
{
  if (res.d_data==NULL || src.d_data==NULL) {
...
#if 1
  dim3 threads(11*11);
  ComputeOrientationsCONST<<<blocks, threads, 0, siftData.stream>>>(texObj, siftData.d_data, octave);
#else
  dim3 threads(256); 
...
double ExtractSiftDescriptors(cudaTextureObject_t texObj, SiftData &siftData, float subsampling, int octave)
{
  ...
  dim3 threads(16, 8);
#ifdef MANAGEDMEM
  ExtractSiftDescriptorsCONST<<<blocks, threads, 0, siftData.stream>>>(texObj, siftData.m_data, subsampling, octave);
#else
  ExtractSiftDescriptorsCONSTNew<<<blocks, threads, 0, siftData.stream>>>(texObj, siftData.d_data, subsampling, octave);
#endif
...
#else
  OrientAndExtractCONST<<<blocks, threads, 0, siftData.stream>>>(texObj, siftData.d_data, subsampling, octave);
#endif
...
double LowPass(const SiftData &siftData, CudaImage &res, CudaImage &src, float scale)
{
...
double LaplaceMulti(const SiftData &siftData, cudaTextureObject_t texObj, CudaImage &baseImage, CudaImage *results, int octave)
{
...
#else
  FindPointsMultiNew<<<blocks, threads, 0, siftData.stream>>>(sources->d_data, siftData.d_data, w, p, h, subsampling, lowestScale, thresh, factor, edgeLimit, octave);
#endif
#endif
